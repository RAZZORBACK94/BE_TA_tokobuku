...
*/
const upload = require(`./upload-image-buku`).single(`image`)
    ...
    upload(request, response, async error => {
        ...
        */
        if (request.file) {
            ...
            const oldImage = selectedBuku.image
            ...
            dataBuku.image = request.file.filename
        }
        /** execute update data based on defined id event */
        bukuModel.update(dataBuku, {
            where: {
                id_buku: id_buku
            }
        })
...
exports.deleteBuku = async (request, response) => {
    ...
    const id_buku = request.params.id_buku
    /** -- delete image file -- */
    /** get selected event's data */
    ...
    bukuModel.destroy({ where: { id_buku: id_buku } })
        .then(result => {
            /** if update's process success */
            return response.json({
                success: true,
